...
select royalty_calc.au_id, round(sum(royalty_calc.sales_royalty) + royalty_calc.advance, 1) AS PROFIT
from (
...
select ap.au_id, ap.title_id, round(sum(ap.sales_royalty), 2) + ap.advance profit
